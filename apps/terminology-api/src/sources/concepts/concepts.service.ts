...
import { Concept} from '@kuunika/terminology-interfaces'
...
export class ConceptsService {
  ...
  async getConceptDescription(sourceId: string, conceptId: string) {
    const source = await this.oclService.requestCategory(sourceId);
    const concept = await this.oclService.requestConcept(source.extras.Route, conceptId);
